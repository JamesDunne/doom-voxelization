...
func (wad *WAD) FindLumpBetween(name string, start uint32, end uint32) *Lump {
	absEnd := wad.EndIndex()
	if end > absEnd {
		end = absEnd
	}
...
func (wc *WADCollection) FindLumpBetween(name string, start string, end string) (lump *Lump) {
	for _, wad := range wc.Ordered {
		...
		lump = wad.FindLumpBetween(name, startIndex, endIndex)
		if lump != nil {
			return
		}
	}
